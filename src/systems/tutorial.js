...
function makeSteps() {
    return [
        // --- Storyline intro ---
        {
            type: 'narrative',
            speaker: 'COMMAND',
            text: 'Captain, this is COMMAND. Welcome aboard MV Eternal Horizon.',
            letterbox: true,
            showPrompt: true,
            allowSteering: false,
            scrolling: false,
        ...
        },
        {
            ...
            runCollision: true,
            onEnter(game, tut) {
                tut._spawnEntity(game, 'powerup', game.tanker.x, 80, { powerupType: 'ceasefire' });
            },
            checkComplete(game, tut) {
...
export class Tutorial {
    ...
    update(delta, game) {
        ...
        if (step.runIronBeam) {
            game.ironBeam.update(delta, game.pools.drone, game.tanker, game.particles, game.audio, releaseEntity);
        } else {
            game.ironBeam.update(delta, { forEach: () => {} }, game.tanker, game.particles, game.audio, releaseEntity);
        }
        ...
        game.terrain.update(delta, game.tanker.z, game.difficulty.getStraitHalfWidth());
