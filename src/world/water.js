...
const WATER_FRAGMENT = `
    ...
    void main() {
        ...
        vec3 mid = vec3(0.06, 0.30, 0.42);
        vec3 surface = vec3(0.12, 0.45, 0.52);
        vec3 highlight = vec3(0.30, 0.60, 0.68);
        ...
        float fresnel = pow(1.0 - abs(vWave), 2.0) * 0.15;
        color += highlight * fresnel;
        ...
        #else
        float foam = smoothstep(0.72, 0.85, n1 * 0.8 + vWave * 0.3);
        #endif
        vec3 foamColor = vec3(0.55, 0.70, 0.72);
...
export class Water {
    constructor(scene) {
        ...
        const segs = quality.settings.waterSegments;
        const geo = new THREE.PlaneGeometry(CONFIG.WATER_PLANE_SIZE, CONFIG.WATER_PLANE_SIZE, segs, segs);
        geo.rotateX(-Math.PI / 2);

        // Enable third noise octave on medium+ quality
        const defines = {};
        if (quality.tier !== 'low') {
            defines.HIGH_DETAIL = '';
        }
